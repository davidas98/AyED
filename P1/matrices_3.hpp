...
// Comprueba si un número es cero	
	
	bool zero(matrix_item_t a, double precision);

// Filtra los resultados de la matriz según un patrón "it"
	
	void filtra(matrix_t& M, matrix_item_t it, double precision);

// Intercambia filas por columnas de la matriz

	void trasponer(void);

	matrix_t suma(double precision);
